...
sealed class PoorMansTerminal
{
    ...
    public async Task<int> RunAsync()
    {
        ...
        var initResult = await InvokeAsync("if (Test-Path 'C:\\run\\my\\prompt.ps1') { . 'C:\\run\\my\\prompt.ps1' -silent } else { . 'C:\\run\\prompt.ps1' -silent }; $PWD.Path");
        ...
        while (true)
        {
            ...
            var wrapped = $"{widthCmd}; if (Test-Path 'C:\\run\\my\\prompt.ps1') {{ . 'C:\\run\\my\\prompt.ps1' -silent }} else {{ . 'C:\\run\\prompt.ps1' -silent }}; Set-Location '{escapedPath}'; {trimmed}; Write-Output \"@@FKH_PWD:$($PWD.Path)\"";
